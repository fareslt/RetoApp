sap.ui.define([
    ...
    return Controller.extend("com.sap.customapp.project1.controller.OrderBurger", {
        ...
        setPaymentMethod: function (oEvent) {
            debugger;
            var QrCodeCont = this.getView().byId("idHTMLContent").getContent();
        ...
        OnChangeCardNbr: function (oEvent) {
            debugger;
            var imageLogo = this.getView().byId("cardBankId");
            var nbr = oEvent.getParameters()["value"];
            if (nbr !== "") {
                var type = this.getCardType(nbr);
                var model = this.getOwnerComponent().getModel("CardList");
                var aData = model.getData();
                var indice = this.findObjectByKey(aData, "cardType", type);
                var path = "CardList>/" + indice + "/imageLink";
                imageLogo.setModel(model, "CardList");
                imageLogo.bindProperty("src", path);
            } else {
                imageLogo.unbindProperty("src");
            }
        },
        ...
        testActivation: function (oEvent) {
            debugger;
        }
